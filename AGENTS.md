...
- 該当する P0/P1 の指摘は件数を理由に省略せず、見つかったものはすべて報告する。
- セキュリティ、データ破壊、認証・認可の欠落、既存機能を壊す挙動変更を優先する。
- テスト・ドキュメント不足は、P0 または P1 に該当する場合だけ指摘する。
- typo や書式だけの指摘は原則として行わない。
- P0/P1 の判定では、被害の大きさに加えて発生経路の実在性も判断材料にする。外部からの入力・攻撃者が制御しうる入力は、仕様上妥当な書式を広く想定して指摘してよい。一方、そのリポジトリ自身が生成・管理し外部入力を受けない設定ファイル等（実際の運用履歴を通じて一貫した書式のみが使われているもの）については、対象リポジトリで実際に発生しうる具体的な経路を示せない理論上の書式差異までは指摘しない。
- 同一関数・同一箇所に起因する、原因が共通する複数の指摘（同種の書式バリエーション漏れ等）は、見つかった時点でまとめて1回で報告する。後続レビューで1件ずつ段階的に追加指摘しない。
