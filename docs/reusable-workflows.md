...
| composite action | `actions/<用途名>/action.yml` | CI のジョブの中でステップとして呼ばれる（[ADR 0003](adr/0003-consolidate-verification-into-single-job.md) 決定6）。 |
| composite action の個別仕様 | `docs/actions/<用途名>.md` | composite action の目的、入力、前提を定義する。 |
...
- composite action は配布先・このリポジトリ自身ともに `uses: hasegawa496/.github/actions/<用途名>@main` を使う。ただし `ci.yml` はリポジトリ固有のため、このリポジトリ自身の `.github/workflows/ci.yml` だけは `uses: ./actions/<用途名>` のローカル参照にする。
...
5. `repo-ops` の `apply` で配布する。既存の `CI` は保持し、存在しない場合は標準 `CI`（checkout と ShellCheck composite action）を配置する。
...
## 提供する composite action

- [ShellCheck](actions/shellcheck.md)
