...
- 処理:
  ...
  5. Project の `Priority` / `Size` フィールド（Single select）に正規化した値を設定する
...
- 失敗時の挙動: **ハード失敗せず、Issue にフォールバックコメントを付けて
  `exit 0` で正常終了する。** 優先度/Size が取得できない場合でも、Project への
  追加（4.）は独立して実行する。フィールド更新（5.）だけがスキップされる。
  ...
  - 見出しが本文に無い / 値が空 → 「優先度/Size を取得できませんでした」
    （この時点で Project への追加は完了済み）
  - Project にフィールドが無い（名前不一致） → 「必要なフィールドが見つかりません」
  - フィールド更新の API 呼び出し失敗（権限不足など） → 個別のコメント
